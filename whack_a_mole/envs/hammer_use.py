...
class HammerUseEnv(PickupEnv):
    ...
    def compute_dense_reward(self, achieved_goal, goal, info=None):
        distance = float(self.check_distance(achieved_goal, goal))
        ...
        reward -= self.horizontal_scale * horizontal_distance

        if vertical_offset > 0.0:
            reward += 0.5 * np.exp(-6.0 * horizontal_distance)

        downward_speed = 0.0
        if info is not None:
            downward_speed = max(0.0, -float(info.get("hammer_tip_velocity_z", 0.0)))
        if horizontal_distance < self.strike_ready_radius:
            reward += self.downward_scale * downward_speed
